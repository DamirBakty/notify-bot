...
from bot import get_bot_updater, get_bot_handler, get_bot_logger


async def main(bot, chat_id, auth_token):

    logger = logging.getLogger()
    long_polling_url = 'YOUR_LONG_POLLING_URL'
    headers = {
        'Authorization': auth_token
    }

    updater = await get_bot_logger(bot, chat_id)
    ...
    try:
        while not stop_event.is_set():
            try:
                a = 1 / 0  # Example error to be handled
                if timestamp:
                ...
                if new_attempts:
                    for attempt in new_attempts:
                        ...
                        if is_negative:
                            ...
                            status = 'Преподателю всё понравилось, можно приступать к следующему уроку!\n\n'
                        ...
                        logger.info(
                            f'Sent Telegram message. Duration: '
                ...
                logger.debug(f'HTTP error. Status: {err}')
            except Exception as e:
                logger.debug(e)
    finally:
        await updater.stop()
